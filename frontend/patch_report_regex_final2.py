...
new_code = """function exportReport(record: RecordItem) {
  ...
  function formatValue(key: string, val: unknown): string {
    if (typeof val === 'boolean') {
      return val ? '是 (Yes)' : '否 (No)';
    }
    if (typeof val === 'number') {
      if (['sand_ratio', 'sandRatio', 'admixture_pct'].includes(key) && val > 1) return val.toFixed(2) + ' %'; 
      if (['sand_ratio', 'sandRatio', 'admixture_pct', 'beta_s'].includes(key) && val <= 1) return (val * 100).toFixed(2) + ' %';
      if (['design_strength', 'designStrength', 'fcu0', 'predict_strength', 'predictStrength', 'sTargetStrength'].includes(key)) return val.toFixed(1);
      if (['densityCorrectionFactor', 'alpha', 'admixture_ratio', 'admixtureRatio', 'sandBinderRatio'].includes(key)) return val.toFixed(3);
  ...
  function renderKV(obj: Record<string, unknown>, indent = 0, isRoot = true): void {
    for (const [key, val] of Object.entries(obj)) {
      ...
      if (isRoot && (key === 'trial_data' || key === 'original_request' || key === 'inputs' || key === 'design_data')) {
        let rootLabel = '试配数据 (计算结果等)';
        if (key === 'inputs' || key === 'original_request') rootLabel = '输入参数';
        if (key === 'design_data') rootLabel = '设计数据/参数';
        rows.push(`<tr><td class="kv-key section-sub" colspan="2" style="padding-left:${indent * 12 + 12}px"><b>⬇ ${rootLabel}</b></td></tr>`);
        renderKV(val as Record<string, unknown>, indent + 1, false);
      } else if (isRoot && (key === 'calculated' || key === 'calculated_mix' || key === 'outputs' || key === 'design_mix' || key === 'mixProportion')) {
        let rootLabel = '计算结果 (模型侧)';
        if (key === 'design_mix' || key === 'mixProportion') rootLabel = '配合比结果';
        rows.push(`<tr><td class="kv-key section-sub" colspan="2" style="padding-left:${indent * 12 + 12}px"><b>⬇ ${rootLabel}</b></td></tr>`);
        renderKV(val as Record<string, unknown>, indent + 1, false);
...
<div class="page-container">
  ...
  const url = URL.createObjectURL(url); // fixed below
...
# wait, I made a typo `URL.createObjectURL(url)` instead of `URL.createObjectURL(blob)`, let's fix it dynamically here:
text = text.replace("URL.createObjectURL(url); // fixed below", "URL.createObjectURL(blob);")
