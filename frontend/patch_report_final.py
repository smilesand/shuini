...
# extract everything up to exportReport
match = re.search(r'(// ── Export report ────────.*?\n)(function exportReport.*?\s\s\})(\n\n  onMounted)', text, flags=re.DOTALL)
if not match:
    print("Could not find exportReport!")
    sys.exit(1)

new_exportReport = """function exportReport(record: RecordItem) {
  ...
  const keyMap: Record<string, string> = {
    ...
    mw: '水 (W)',
    ma: '外加剂 (A)',
    water_binder_ratio: '水胶比 (W/B)',
    sand_ratio: '砂率 (Sp)',
    design_strength: '设计强度',
    fcu0: '配制强度',
    batchVolume: '试拌体积',
    densityCorrectionFactor: '密度校正系数 (δ)',
    calculatedDensity: '计算表观密度',
    measuredDensity: '实测表观密度',
    lab_mix: '实验室配合比',
    design_mix: '设计配合比',
    trial_mix: '试配配合比',
    variant_wb_plus: '水胶比 +0.01',
    variant_wb_minus: '水胶比 -0.01',
    variant_sf_plus: '硅灰 +5%',
    variant_sf_minus: '硅灰 -5%',
    strengthMixes: '强度试配序列',
  };
...
<div class="page-container">
  ...
  <table>
    <thead>
      <tr><th>参数名称 (中英文缩写)</th><th>数值</th></tr>
    </thead>
    <tbody>
      ${rows.join('\\n')}
    </tbody>
  ...
  <div class="footer">
    本记录由 水泥配合比计算系统 自动生成 · 打印时间：${printTimeStr}
  </div>
...
full_text = text[:match.start(2)] + new_exportReport + text[match.end(2):]
Path(path).write_text(full_text, encoding='utf-8')
print("exportReport successfully replaced with beautiful print version!")